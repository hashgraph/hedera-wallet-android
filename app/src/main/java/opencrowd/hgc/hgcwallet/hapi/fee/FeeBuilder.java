...
public class FeeBuilder {
	...
	public int getCommonTransactionBodyBytes(Transaction tx) {
		...
		int commonTxBytes = 3 * LONG_SIZE + (LONG_SIZE + INT_SIZE) + 3 * LONG_SIZE + LONG_SIZE + (LONG_SIZE + INT_SIZE) + BOOL_SIZE + APIRequestBuilder.getTxnBody(tx).getMemo().getBytes().length;
	...
	public long getGas(Transaction tx) {
		long gas = 0;
		TransactionBody body = APIRequestBuilder.getTxnBody(tx);
