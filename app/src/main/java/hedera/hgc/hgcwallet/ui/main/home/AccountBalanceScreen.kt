...
import hedera.hgc.hgcwallet.ui.onboard.Bip32Migration.Bip39MigrationScreen
...
class AccountBalanceScreen : Screen<AccountBalanceView>(), AuthListener {
    ...
    data class Params(val accountList: List<Account>, var txnList: List<TxnRecord>)

    private val param: Params
    private val disposables = CompositeDisposable()

    init {
        param = Params(DBHelper.getAllAccounts(), listOf())
    }
...
class AccountBalanceView(context: Context, val param: AccountBalanceScreen.Params) : BaseScreenView<AccountBalanceScreen>(context) {
    ...
    init {
        ...
        if (Singleton.canShowBip39MigrationMessage())
            bip39View.setOnClickListener { screen.navigator.goTo(Bip39MigrationScreen()) }
        else
        ...
        mEditTextNickName = findViewById<EditText>(R.id.edittext_nick_name)?.apply {
            setOnFocusChangeListener { view, b ->
                if (!b) currentAccount?.let { screen?.updateAccountName(it, text.toString()) }
            }
        }
    ...
    internal class OverViewPagerAdapter(private val context: Context, val accountsList: List<Account>, private val listener: OverViewPagerAdapterListener? = null) : PagerAdapter() {
