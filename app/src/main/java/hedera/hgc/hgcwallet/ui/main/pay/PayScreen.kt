...
class PayScreen(fromAccount: Account, type: PayType = PayType.Pay) : Screen<PayScreenView>(), AccountListScreen.AccountPickerListener, ContactListScreen.ContactPickerListener, QRScanListener {
    ...
    constructor(fromAccount: Account, toAccount: PayRequest, type: PayType) : this(fromAccount, type) {
        param.apply {
            ...
            amountStr = Singleton.toCoins(toAccount.amount).toString() + ""
        }
            ...
            when (param.type) {
                PayType.Pay -> TransferRequestParams.fromBarCode(result)?.let { requestParams ->
                    ...
                    param.apply {
                        ...
                        toName = if (requestParams.name == null) "" else requestParams.name
                        amountStr = Singleton.toCoins(requestParams.amount).toString() + ""
                        if (requestParams.note != null) {
                            notes = requestParams.note
                            addNote = true
                        }
                        showToAccount = true
                    }
    ...
    internal fun onPayButtonClick() {
        ...
        if (error == null) {
            ...
            val task = TransferTaskAPI(param.fromAccount, toAccountID!!, param.notes, param.toName, Singleton.toNanoCoins(amount), param.fee)
    ...
    internal fun onPayExchangeButtonClick() {
        ...
        if (error.isEmpty()) {
            activity.showActivityProgress("Please wait")
            val txn = APIRequestBuilder.requestForTransfer(param.fromAccount, toAccountID!!, Singleton.toNanoCoins(amount), param.notes, param.fee, Node.placeholderNode().accountID(), forThirdParty())
            disposables.add(
                    (+Single.fromCallable {
...
class PayScreenView(context: Context, val param: PayScreen.Params) : BaseScreenView<PayScreen>(context) {
    ...
    fun setNoteTextFieldVisible(visible: Boolean) {
        ...
        noteText?.visibility = visibility
        noteEditText?.visibility = visibility
    }
